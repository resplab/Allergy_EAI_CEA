...
# Build parameter for base model
...
# Build cost and utility for each state in base model
...
# Build strategy and define initial number of patients
...
# run Model X 100, 500, 1000 mortality
...
# run model from societal perspective
...
summary(allergy_mod_dr_3)

```

# Get output result and table after simulation (with out PSA)
...
## PSA graph-base model 

```{r}

load("psa_result_base")
...
psa_analysis<-psa_result_1000$psa

cea_analysis<-psa_analysis%>% 
  arrange(.strategy_names, .index) %>%
  group_by(.index) %>%
mutate(cost_diff = (.cost-lag(.cost))/10000, utility_diff = (.effect-lag(.effect))/10000)
...
load("psa_result_100")
load("psa_result_500")
load("psa_result_1000")
load("psa_result_dr_0")
load("psa_result_dr_3")
load("psa_result_societal")
