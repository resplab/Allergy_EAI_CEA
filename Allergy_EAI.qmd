...
tmp %>%   
  ...
  summarize(.groups = "keep", avg=mean(count), sum=sum(count)) 
...
tornado_plot <- function(df, refer_value){
  ...
  df<-df %>%  group_by(.par_names, Boundary) %>%
    summarise( .groups = "keep", cost_diff =.cost[.strategy_names == "ED_transfer"] - .cost[.strategy_names == "watch_wait"],
...
tornado_plot(allergy_dsa$dsa, cost_per_year_QALY_saved_ref)
